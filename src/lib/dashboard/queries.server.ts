...
import { formatRequestDetails, loadShiftPreviewsForRequests, mapRequestRow } from "@/lib/requests/utils";
...
export async function fetchDashboardTodayShifts(
  ...
  const [shiftsResult, checkinsResult] = await Promise.all([
    supabase
      .from("shifts")
      .select("id, employee_id, start_time, end_time, employees ( first_name, last_name ), positions ( name )")
      .eq("organization_id", organizationId)
      .eq("shift_date", today)
